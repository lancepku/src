...
!HL=abs(cord(Z600,nx/2,2))
!HG=HL+2*D_axial  ! should redescribe the HG
!Print *,'HL',HL,'HG',HG

!if (Zamc.lt.6.e3) then
!rate_inject_HG = 1.0e-9
!else if (Zamc.ge.6.e3)then
!rate_inject_HG = 1.0e-9*6.e3/(HL+0.5*HG)
!endif
!if (rate_inject_HG.ge.1.01e-9) then
!rate_inject_HG = 1.0e-9
!end if
!rate_inject=rate_inject_HG
nelem_inject = int(Z600)
